...
This is $A\mathbf{x} = \mathbf{b}$, where $A$ is the coefficient matrix, $\mathbf{x}$ is the unknown vector (currents), and $\mathbf{b}$ is the source vector (voltages).
